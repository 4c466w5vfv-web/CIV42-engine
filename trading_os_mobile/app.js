...
const state = {
  ...
  signalIndex:0,
  signals:[
    {symbol:'AVGO', cluster:'Semiconductor', bigView:true, trend:true, zone:true, trigger:true},
    {symbol:'GC', cluster:'Metals', bigView:true, trend:true, zone:true, trigger:true},
    {symbol:'AMD', cluster:'Semiconductor', bigView:true, trend:true, zone:false, trigger:false},
    {symbol:'JPM', cluster:'Financials', bigView:true, trend:true, zone:true, trigger:true}
  ],
...
function clusters(){
  const out={};
  state.positions.forEach(p=>out[p.cluster]=(out[p.cluster]||0)+p.openRisk);
  return out;
}
...
function signalDecision(sig){
  ...
  const needed = DEFAULT_THESIS_RISK;
  if(availableRisk()<needed) return {status:'BLOCK', reason:'portfolio risk full'};
  if(clusterRisk(sig.cluster)+needed>MAX_CLUSTER_RISK) return {status:'BLOCK', reason:'cluster risk full'};
  return {status:'ENTER', reason:'risk + setup approved'};
}
function setStateBadge(el,status){
  el.textContent=status; el.className='state '+status.toLowerCase();
}
function render(){
  ...
  const decision=signalDecision(currentSignal());
  ...
  const btn=document.getElementById('executeSlot'); btn.disabled=decision.status!=='ENTER';
  const nextSlot=state.slots.findIndex(x=>!x); btn.textContent=nextSlot<0?'3분할 완료':`승인된 ${nextSlot+1}차 진입 기록`;
  ['slot1','slot2','slot3'].forEach((id,i)=>document.getElementById(id).classList.toggle('done',state.slots[i]));
...
document.querySelectorAll('.tab').forEach(btn=>btn.addEventListener('click',()=>{
  ...
  document.querySelectorAll('.screen').forEach(x=>x.classList.toggle('active',x.dataset.screen===btn.dataset.tab));
}));

document.getElementById('simulateNext').addEventListener('click',()=>{state.signalIndex++;state.slots=[false,false,false];render()});

document.getElementById('executeSlot').addEventListener('click',()=>{
  const sig=currentSignal(), decision=signalDecision(sig); if(decision.status!=='ENTER') return;
  const idx=state.slots.findIndex(x=>!x); if(idx<0) return;
  state.slots[idx]=true;
  const portions=[0.30,0.225,0.225];
  let p=state.positions.find(x=>x.symbol===sig.symbol);
  if(!p){p={symbol:sig.symbol,side:'LONG',cluster:sig.cluster,r:0,openRisk:0,protected:false};state.positions.push(p)}
  p.openRisk=+(p.openRisk+portions[idx]).toFixed(3);
  state.journal.push({symbol:sig.symbol,result:`ENTRY ${idx+1}`,note:`${[40,30,30][idx]}% slot · thesis risk ${pct(p.openRisk)}`,good:true});
  render();
});

document.getElementById('addDemoExit').addEventListener('click',()=>{
  const p=state.positions.find(x=>x.openRisk>0); if(!p) return;
  p.openRisk=0;p.protected=true;p.r=2.8;state.equity+=2100;state.peakEquity=Math.max(state.peakEquity,state.equity);
  state.journal.push({symbol:p.symbol,result:'+2.8R',note:'20EMA partial → 50SMA final · risk recycled',good:true});render();
});
